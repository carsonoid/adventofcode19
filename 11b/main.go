...
func main() {
	...
	c.input <- 1 // Start on white panel
	c.color = 0
	go c.start()
	...
	y := 0
	facing := 1
	for color := range c.output {
		direction := <-c.output
		p := panel{x, y, color} // get current panel, set color
		// fmt.Printf("%d, %d\n", color, direction)
		// fmt.Printf("ROBOT START ON %s, color %d, direction %d, facing %d\n", p.String(), color, direction, facing)
		panels[p.String()] = p //  upsert map of known points
		// fmt.Printf("PANELS %v\n", panels)
		...
		// Get panel after moving
		p = panel{x, y, 0}
		if existing, ok := panels[p.String()]; ok { // already visted
			c.input <- existing.Color
		} else {
			c.input <- 0
		}
	}
	...
	for y = 0; y > -10; y-- {
		for x = 0; x < 50; x++ {
			p := panel{x, y, 0}
			if existing, ok := panels[p.String()]; ok {

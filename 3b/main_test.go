...
func Test_getFewestWireCrossSteps(t *testing.T) {
	tests := []struct {
		name  string
		inst1 []instruction
		inst2 []instruction
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := getFewestWireCrossSteps(tt.inst1, tt.inst2); got != tt.want {
				t.Errorf("getFewestWireCrossSteps() = %v, want %v", got, tt.want)
			}
		})
	}
}
